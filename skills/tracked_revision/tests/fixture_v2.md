...
2. Ask the AI to revise one section at a time, so that each change can be reviewed against the protocol.
...
| Step | Tool | Output |
|------|------|--------|
| Draft | IDE + AI | draft.md |
| Render | pandoc | draft.docx |
| Review | Google Docs | comments, suggestions |

# Discussion

AI-assisted writing is most useful when the project files are explicit and the revision target is narrow. For example, asking the AI to revise only the Background section is easier to review than asking it to rewrite the whole manuscript. The researcher remains responsible for the argument, the evidence, and the final wording. When AI output conflicts with the protocol, the protocol takes precedence and the discrepancy is recorded in the project log.
